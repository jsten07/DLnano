...
path_true_data <- "~/data/Mossul/prepared/multitemporal/128/true"
path_false_data <- "~/data/Mossul/prepared/multitemporal/128/false"
...
calculateDifs = function(rasterStack) {
  band1 = rasterStack[,,1] - rasterStack[,,5]
  band2 = rasterStack[,,2] - rasterStack[,,6]
  band3 = rasterStack[,,4] - rasterStack[,,8]
  newStars= abind(band1, band2, band3,  along= 3)
  return(newStars)
}
...
training_dataset$img <- lapply(training_dataset$img, calculateDifs)
...
validation_dataset$img <- lapply(validation_dataset$img, calculateDifs)
...
#test = raster("~/data/Mossul/predictions_blockwise/out/_out_1.tif")
#plot(test)
...
data_original$img <- lapply(data_original$img, calculateDifs)

completeData = dl_prepare_data(data_original, F, F, c(128,128,3))
...
result_map <- raster("./data/Mossul/predictions_blockwise/out/mosaic.tif")%>%readAll()
